...
namespace SampleJsonConverterCode
{
    ...
    public class ClassWithDictOfStringsByIntJsonConverter : JsonConverter<ClassWithDictOfStringsByInt>
    {
            ...
            while (true)
            {
                ...
                switch(reader.TokenType)
                {
                    ...
                    case JsonTokenType.EndObject:
                        if (SomeStringsByInt == null) throw new JsonException("ClassWithDictOfStringsByInt property SomeStringsByInt is missing");
                    ...
                    case JsonTokenType.PropertyName:
                        switch(reader.GetString())
                        {
                            case nameof(ClassWithDictOfStringsByInt.SomeStringsByInt):
                                SomeStringsByInt = ReadDictString<int>(reader,s => int.Parse(s));
                                break;
                            case nameof(ClassWithDictOfStringsByInt.MaybeSomeStringsByInt):
                                MaybeSomeStringsByInt = ReadDictString<int>(reader, s => int.Parse(s));
                                break;
                            default:    
                                break;
                        }
        ...
        private Dictionary<T1,string> ReadDictString<T1>(Utf8JsonReader reader,Func<string,T1> stringToKeyType) where T1: notnull
        {
        ...
        public override void Write(Utf8JsonWriter writer, ClassWithDictOfStringsByInt value, JsonSerializerOptions options)
        {
            ...
            WriteDict<int, string>(writer,
                nameof(ClassWithDictOfStringsByInt.SomeStringsByInt),
                value.SomeStringsByInt,
                (k, v) => writer.WriteString(k.ToString(), v)
                );

            if (value.MaybeSomeStringsByInt != null)
            {
                WriteDict<int, string>(writer,
                    nameof(ClassWithDictOfStringsByInt.MaybeSomeStringsByInt),
                    value.MaybeSomeStringsByInt,
                    (k, v) => writer.WriteString(k.ToString(), v)
                    );
            }
